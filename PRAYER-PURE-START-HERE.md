# PRAYER-ONLY PURE — START HERE · 10 HITPOINTS · ONE FILE, TWO ROUTES
...
| 601–640 | disjoint | — | **40/40** |
| 801–840 | disjoint | — | **40/40** |
...
2. **"The cliff tracks `cycleOverhead`."** Sweeping `cycleOverhead` 3 / 4 / 5 moves it **not one tick** — the
   ...
   arithmetic, not by that parameter.
...
stall and aborts if anything lands on you — because twelve inventory slots had to go to Guthix rest. I measured
...
the next. The trade I measured was real — it was a property of a route that was dying, not of the ring handling.
...
**Standing in the 3×3 entrance never triggers the ice block.** Rendi confirmed it.
...
Consequence for the simulator: **`blockRisk` is now a load-bearing invalidity check rather than a hedge.** A run
...
**Rendi, 21 Aug:** *"at higher health the rock cake does more damage, I believe at 10 it starts at 2 or 3 with
...
| 10 | 10 | **10/30** | — |
| 99 (pure cake, zero nightshade) | 0 | **14/30** | — |
...
Pure cake tops out at 14/30, and raising the tick cap to five hours shows it **stalling at 508 recoil of 520** —
it survives and cannot finish. Zero nightshade is not available at any speed.
...
**Rendi:** *"one fail is -500 nightshade which is months of farming."* Exactly right, and it dominates
...
Rendi, 21 Aug: *the quest fight only happens once; post-quest kill count happens over and over, so the supply
...
|---|---|---|---|---|
| 0 (was shipped) | 904 | 30/30 | 35.9 min | — |
...
The arithmetic floor is about **116** — 520 recoil at the full **9** instances an arm, two nightshade an arm.
This route converts **1.7** instances an arm, not 9. **The entire gap is the exit-clearance problem**, because
every instance banked lowers `1 + heal − instances`, which is exactly the number the exit roll has to be
survived on. If supply is the thing stopping post-quest kill count, **that** is the lever — not the descent.
...
# ★ THE ROCK CAKE — asked and answered, and the arithmetic behind the question was right
...
**The arithmetic is exactly right.** `postHp = 10 − instances`. At the full capacity of **9 instances the dump
...
**But the longer stall cannot get there, and the reason is that the lever is self-limiting.** Measured:
...
**The shipped 100% quest route is the floor tank. It carries no nightshade, no belladonna and no rock cake** —
the floor *is* the descent. The nightshade farm is a **post-quest-only** problem, and post-quest it is ~1200
...
Recoil is **400 on every row**. The ring costs nothing and buys 18/160 on route A and 27/160 on route B.
...
Every death on route A across 80 seeds lands on **t63 or t97** — the second or third icicle. Past t97 the run
...
roll instead. The diagnosis is right; the fix is not. At a 4-tick window route A goes **142/160 → 0/160** and
...
written into the sim as a comment so it is not tried a third time.** The fix is spatial.
...
on route A a slipped click costs the run, on route B it costs hitpoints you eat back. It is 157/160 perfect and
**the best of the three at every non-zero slip rate**. Route A only wins at literally zero misclicks, by three
...
| 3–8 | 24 · 0/40 | — | 188 · 0/40 | — | 88 · 0/40 |
| **6–10** (wiki) | 1 · 0/40 | — | 146 · 0/40 | — | 2 · 0/40 |

At **floor 3–5 they tie** — both 40/40, and B is 400 ticks faster. At **floor 3–6 route A is 40/40 and route B
is 6/40**, and you will not know the floor's real maximum until you are standing on it. **A is the only route
robust to the answer.** Above a maximum of 6 neither works — the cliff between 6 and 7 is unchanged.
...
# ★★★ AND WHEN YOU PUT A HUMAN ON IT, THE ROUTE CHANGES
...
**Route A wins only at zero.** From one slip in four hundred onward the floor tank is the better plan, and by
...
>
> The **conditional** flick (`protMagic 3`) — which is exactly what route B runs — was never reading
> `slipChance`. Only the held-flick branch (`protMagic 2`) and the arm click paid the tax. Route B therefore
> read **157/160 at every slip rate up to 2%**, i.e. misclick-proof. That was a branch that forgot to pay the
> tax, not a property of the route. **Fixed; the row above is the corrected one.** Any earlier statement from me
...
★ Rendi: *"I can use unnoted rings and not refill for the hybrid method — it will just take like 10 inventory
...
| price | — | **~4 recoil a run on route A** (≈11 ticks) |

40 charges a ring × **10 rings = exactly the 400 hitpoints of the quest variant**; the run never went dry on 10,
so the 11th is margin, and your arithmetic was right. The ~4 lost recoil is a ring crumbling **mid-stall**: the
...
| *(for reference, at 20 hp)* | — | 400 recoil · 38/40 · t885 |
...
3. **The dump may land only on 1 with a point in hand, or above the maximum floor roll.** Check the tile at
   **dump AND dump+1**.
4. **Never step onto a tile you cannot land on** — the mover checks where it lands, never what it crosses.
...
could fire."* **The real lever is spending the point on the tick it arrives; bonus 0 only achieves that while
the cycle is short.** Sweep it rather than fixing it.

---

## ✅ THE PROC ORDER — ★ Rendi described it exactly, and the engine already matches

★ Rendi: *"rock cake into an inventory action stall… the rock cake could hit even a zero on my one HP and still
...
mover checks the tile it lands on and not the ones it crosses) and *be on tiles 1–9 when the special fires*.
...
**The idea is forced by the two routes' own shapes.** Method A converts ~1.6 instances per proc and then sits
...
kit**. Two things survive: **the 1-tick easter ring is load-bearing** (crate ring alone is 6 recoil), and adding
...
★ Rendi asked for this check directly, and it was the right thing to ask: three rounds of movement corrections
all touched the mover, and the one rule that must never bend is that **a stall takes your feet as well as your
hands.** Audited across both routes, 40 runs:
...
★ Rendi: *"You can eat and move at the same time, so that cooldown between the sweets doesn't impede movement
really at all."*
...
**Over half the fight is spent standing still while eating.** Those are free movement ticks being thrown away —
the climb back from a floor instance ought to double as the walk to the next patch or into the entrance, and it
...
took the quest route from **400 / 29-40 to 246 / 2-40**, and sweets-only from **400 / 22-40 to 27 / 0-40** —
because a tile adjacent to live ice is precisely where the next auto seeds you, and at climbing hitpoints that
...
★ Rendi: *"Only if post-quest floor damage hits 10 or more would we run out of rests. Sweets can supplement all
...
★ Rendi: *"Both methods should work for the quest variant because the floor only damages 3–5 during quest."*
...
3. **`stepOn` was 14 on a 10-hitpoint account** — a default carried over from the 20-hitpoint model, so the
   "climb back to here before returning to the ice" threshold was unreachable and the route almost never went
   back. **Set to 10.**
...
★ Rendi: *"unsure what it is post quest."* The wiki says the icy pool deals **6–10 per tick**, verbatim, and you
...
- **If it is 6–10 as the wiki says, neither works at 10 hitpoints** and the frozen cap is the only mechanism
  with a route through, because it turns any roll into a 1.
...
⛔ **And I had the post-quest presets mislabelled.** ★ Rendi: *"post quest is 7-10, pre quest is 3-5."* I had two
...
# ★★ THE TWO ROUTES — everything else on this page is workings
...
★ Rendi: *"What does 'no ring' specifically mean in your responses?"* Fair. I had been calling the fixed tile
route a **ring**, on a page whose whole premise is a **ring of recoil** and a **ring of suffering**. Two
completely different things sharing one word, and I never noticed. **It is called the LOOP now**, everywhere —
the dropdown, the parameter, the notes. When I said "ring-free" I meant "the planner picks a tile every tick
...
★ Rendi: *"look into seeing if this is possible with cooled rock cake guzzle descents rather than nightshade, as
that's harder to get."*

| descent tool | route A | hybrid |
|---|---|---|
...
which is free and needs no consumable at all. It is also why route B is the one that does not need belladonna,
...
★ Rendi: *"If there are always free tiles now on the descent I'm not sure why it's dying in any of these. Still
need easiest most optimized 100% route."*

**He was right that the remaining deaths were avoidable, and the fix was two ticks per block.**

Every surviving death on route B was one shape. Traced, seed 54:
...
## ★★ And it does not trade away the thing that made route B worth running
...
other and here they do not, because **you are never at 1 hitpoint on this route.** A missed click costs
hitpoints you eat back, not the run.

## ⛔ AND THE LOADOUT I SHIPPED DID NOT FIT IN AN INVENTORY

★ Rendi: *"the less rests the better as long as it doesn't kill an insane amount of time."* Sweeping that
...
**`holdBeforeSpec 6` also makes route B 160/160 at every rest count, including zero**, so the bank is purely a
...
**Every piece of this came from Rendi's corrections, and the last one was the big one.**

★ Rendi: *"Tiles should be freeing up and I never should have to run over a floor tile."* **He was right, and
...
**Essentially every patch in the arena was in the entrance, because that is where it stood.** 135 clean tiles
...
**All three parts are load-bearing** — remove any one and it falls to 0-6 in 160. And it is consistent across
disjoint seed blocks, so it is not the seed luck that caught me earlier.

### ⚠ And sweets beat pizza, exactly as Rendi predicted

★ Rendi: *"optimise for lowest rest count though, as pizza will take some inventory at 2 bites per slot."*
...
# ★ A MECHANIC CORRECTION FROM RENDI — the rest overheals from ANY hitpoints

★ Rendi: *"chocolate cake has 3 bites (more per slot) and does 5 hp, combo eaten into a rest for +10 overheal
...
10 bar when the rest landed on a *full* bar. **His rule is now the default** (`restOverhealFrom 1`), because he
is authoritative on how the game works and that outranks the score.

**It turned out to help anyway** — route B got materially faster:
...
**Four and a half minutes off route B for free**, because a rest drunk mid-climb now lands its full value instead
of clipping at the bar. Route A and the hybrid are unchanged.

### ⛔ And the burst combo — built, and it never fires

★ Rendi: *"I was thinking you needed an instant heal to 15 in a descent to run over an unavoidable pattern for a
...
**Post-quest it fires zero times a run.** Once the ring is out in the room there is essentially always a clean
neighbour to step to — **the trapped case it was designed for is one the room ring already removed.** So the
answer to *"will I run over a live tile on the descent?"* is: **out in the room, essentially never.** In the
...
I told you route B spends *"184 of 184 doses, zero margin"* and called it the binder. **The first half is true
and the conclusion is wrong.** It spends everything it has because it is opportunistic, not because it needs it.

★ Rendi: *"can we extend the fight and not use as many rests, subbing in sweets as much as possible?"* Swept:
...
# ★ THE EXIT ARITHMETIC — the one thing that IS proven impossible
...
★ Rendi: *"coming out the stall, if the pattern hasn't expired won't it hit after the stall for my full hp?"*
**Yes, and post-quest every row dies — including zero instances, where you walk out at full health.** 1 + 9 = 10,
the floor rolls up to 10. Not a tuning problem, not a filter artefact: **arithmetic.**
...
reason Rendi gave: at 3 instances you exit on 7, and 7 − 5 = 2.

---

# ★★★ RENDI'S GRACE-WINDOW IDEA — it is right, and it moved the blockage

★ Rendi: *"Couldn't we just use a shorter stall at the beginning of an auto attack? And combine that with the
...
> ### ★ SO THE NEXT LEVER IS YOUR OWN IDEA, APPLIED ONE STEP EARLIER
>
...
> plans after the fact. **Still not built** — it is the one untried lever, and everything it needs now exists.

### ★ AND THE CLIMB COULD NOT REACH 15 EITHER — Rendi's pizza idea, built as `redempBoost`

★ Rendi: *"could we pizza + rest dose on these rare descents to go from 1 to 15 in one tick?"* **The reason that
...
> | `exitGrace` — Rendi's +4/+5/+6 window | **unblocked the planner**, 0 → 7 arms a run at 7-10 |
> | `redempBoost` — Rendi's climb to 15 | safe and free on quest, +1 kill at 3-7, nothing at 7-10 |
>
...
>
> **What is NOT proven is that the fight is impossible.** The untried lever is the descent scheduler, and it is
...
★ Rendi: *"we have to stand on the ice floor as it expires out of the stall, or else the tick after Redemption
...
across the dump tick *and the tick after it*, and only on this route:
...
### ⚠ `rotateArm` — built, measured, and it is not the lever
...
★ Rendi's floor figure for the real boss is **7-10**, which supersedes the wiki's 6-10. Both are past the cliff,
...
because there is no roll coming. That is precisely the mechanic Rendi describes, and it is narrower than the
...
> a floor that behaves like the quest one.** At your 7-10 figure, none of the three routes work as modelled.
...
**The farm tiles are decoration.** And route A spends **97% of its ticks inside tiles 1–9**, so it was never
...
Across 640 runs and every configuration above, **every route-A death lands at t63 or t97, and not one lands
after.**
...
**Route A's kill rate is not a survival rate. It is the probability you survive the first ninety-seven ticks** —
the second and third icicle — and the "ring" is a way of scripting that opening. After the third icicle the run
...
### Route B is different, and I can't explain that either

On the floor tank the two halves are **both** load-bearing, which is the opposite of route A:
...
# ⛔ THREE THINGS RENDI CAUGHT IN THE BUILD, AND ALL THREE WERE REAL

### 1. "The sim is using Protect from Magic on the Redemption route too, not just the hybrid — is that correct?"

**Yes, and it is load-bearing on every route.** Swept it, 160 seeds:

| `protMagic` | route A recoil | ends | kills |
|---|---|---|---|
| **0 — off** | 2 | t16 | **0/160** |
| **1 — held** | 1 | t16 | **0/160** |
...
flick is what makes any of the three routes exist.**

⚠ What it *costs* is worth saying out loud, because it is counter-intuitive: **a protected auto returns no
recoil.** Route A flicks it **218 times a run** and gets 112 more flicks blocked for want of a free tick. That is
218 hits' worth of output traded for survival — the routes still reach 400 because their damage comes out of the
...
cures on route A.** Seed 1 traced tick by tick: the dump leaves **5 hitpoints** → the reservation for the special
...
hybrid its recoil. **So the hybrid's t97 death is not fixable without turning it into route A.** That is the
...
- **The tick sheet is now the full log you asked for.** ★ Rendi: *"the tick sheet is supposed to include the
  entire logged tick by tick breakdown of the entire fight simulated."* It does. The engine has always recorded a
  frame per tick; nothing was reading them except the scrubber. Every tick of the loaded run is now a row —
  **hitpoints, her hitpoints, which tile, whether you were inside 1–9, whether the prayer was up, prayer points,
...
1. **Route A is cheap.** 58 rest doses of a 184 bank and 58 sweets. It is the *reliability* that is expensive,
   not the supplies.
2. ⚠ **Route B drinks the entire rest bank, every single run — 184 of 184, on 160 seeds out of 160.** It is not
   that it needs exactly 184; it is that it drinks everything it has and then finishes on sweets. That is zero
   margin, and it is why the sweets-only variant takes t3115 instead of t2115: **the rests are what make route B
...
stall routes, so route B read **0 sweets** on its first pass. Route B heals through a different branch. Fixed —
the 498 and 908 above are the corrected figures, and any earlier sweet count from me is withdrawn.
...
Each row sets the route, the ring, the prayer mode, the ring supply and the boss's stats **together**, because
...
```
  Preset  [ ★ QUEST — recommended · Floor tank on the two-tile shuttle  ▾ ]
          ...
          ★ POST-QUEST — recommended · Redemption on the rectangle
            ...
            CONTROL — no ring, router free (the before picture)
          — custom (you have changed something) —
```
...
⚠ **One consequence, recorded because it bit me:** the page now mutates `P` at boot, so any offline harness that
snapshotted `P` at load was capturing the *preset* instead of the file's declared defaults — which made route A
...
The **Plan** dropdown already carried all three routes (Redemption · Hybrid · Floor-tank). The patrol was a
...
**Changing the route moves the ring with it** to the pairing that was measured best — Redemption→1,
...
**So the floor pattern is 6–10 per tick, and the spike's number on that boss is genuinely unpublished.** The sim
...
★ Rendi: *"First step crossing a live tile doesn't actually hit you. You SKIP tiles when you run two tiles at a
...
mover checks the tile it lands on, never the ones it crosses — at 1 hitpoint every crossed tile is lethal."*
...
**So the answer is inventory, and the inventory does not stretch.** Your stated kit — ~25 four-dose plus ~28
...
instances and costs a full climb back. **Method A is unaffected by all of this** — 401 recoil, 40/40, t2713 —
because the frozen cap means it never needs the band at all.

---

## ⛔ TWO THINGS I GOT WRONG, BOTH CAUGHT BY RENDI

### 1. "An arena where every live tile deals 6–10" — WRONG, and it was doing real work in my conclusion

★ Rendi: *"Won't tiles be freeing up to eat sweets on? Patterns go away per the rules in the engine and are free
...
- The escape search allowed a target up to a full movement tick away (2 squares) but **checked only the
  destination**, so the first step could land on a live patch. Traced at t189: 5 hitpoints, target two squares
...
**The conclusion survives, but for the right reason now.** Method B still does not do the post-quest boss —
58 of 520 — and it is not because there is nowhere to stand. It is that **at 15 hitpoints one 6–10 roll drops
...
★ Rendi: *"we have access to 1 and 3 tick stalls."*
...
★ Rendi: *"rock cake doesn't kill at 1 hp, it hits 0 — just confirming."* **Yes, and that is exactly how it is
...
★ Rendi: *"for both of these methods we don't need poison. We're not going to be taking any poison damage,
...
- the **next-event arithmetic** that decides how much slack a cycle has;
...
★ Rendi: *"if the boss somehow does manage to hit over ten in a singular hitsplat, the recoil sends TWO damage
...
★ Rendi: *"going from two to one HP if I somehow regenerate an HP, with a one proccing instead of a zero, and
...
must be about half as long. Here is the arithmetic at 10 hitpoints with a heal of 9:
...
★ Rendi: *"real boss numbers are the non-quest variant only, and not sure if it mentions floor damage numbers.
...
- **⛔ Relaxing that rule was measured and REJECTED** — allowing any dump that merely survives, on the grounds
  ...
  and 40/40 → 0/40. The rule is load-bearing, not conservative.

**The lever that is left is the rotation** — `arm at ice age 105 − L`, so the patch expires exactly at the dump
...
1. ~~**Does staying in tiles 1–9 on the fire tick prevent the unstable ice blocks?**~~
   **✅ ANSWERED — 21 August, Rendi, confirmed in game: standing in the 3×3 entrance never triggers the block.**
   ...
   out-heal recoil. It went the right way. Consequence for the sim: **`blockRisk` is now a load-bearing
...
★ Rendi, and this is the rule the engine was built on: *"you SKIP tiles when you run two at a time — the first
...
and that somewhere was whatever the greedy step happened to land on.
...
survivable. **Measured, 160 seeds: route B 120/160 → 130/160, hybrid 92/160 → 100/160, route A unchanged.**
...
is what it takes. Without that exemption the fix takes route B to 40/40 and `blockRisk` **0 → 4**, which is not
...
correct-sounding and cost route A **160/160 → 112/160**, because most unreachable plans are merely *worse*, not
...
| File | What |
|---|---|
| `osrs-prayer-pure-sim.html` | **`PRAYERPURE-V2`** — 10 hitpoints, both routes on the Route dropdown |
